...
IVAR_TRIM_BASE = 'ivar trim -x {nprocs} -e -b {primer} -i %s -p {out_dir}/%s'
...
def _generate_commands(untrimmed_bams_gz, nprocs, primer, out_dir):
    ...
    out_files = []
    commands = []
    for bam_gz in untrimmed_bams_gz:
        fname_gz = basename(bam_gz)
        fname = fname_gz[:-3]
        bam = bam_gz[:-3]
        prefix_name = bam[:-11] + '.trimmed.sorted.bam'
